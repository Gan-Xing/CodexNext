...
- `full-access` over relay should follow Codex by default; only disable it with `CODEXNEXT_DISABLE_RELAY_FULL_ACCESS=1` if an operator explicitly wants that extra gate
...
- If request comes through relay and asks for `permissionMode: "full-access"`, allow it by default so relay matches Codex; reject only when env `CODEXNEXT_DISABLE_RELAY_FULL_ACCESS=1` is set.
...
13. codex relay full-access 默认允许，只有 CODEXNEXT_DISABLE_RELAY_FULL_ACCESS=1 时才禁用。
...
验收：
未登录打开公网 Web 只能看到登录页；未登录 POST /api/relay/session 返回 401；登录后可看到设备并控制 Codex；退出登录后刷新不能继续控制；设备 revoke 后不能重连；control-devices.json 不含明文 deviceToken；relay full-access 默认允许。
```
